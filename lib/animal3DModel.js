...
function calculateAnimalParamsFromData(data) {
  ...
  const baseSize = 5 + (avgValue * 10) // 5-15cm
  const headRatio = 0.4 + (avgValue * 0.2) // 0.4-0.6 (keeps head proportional)
  const limbLength = Math.max(0.3, Math.min(1.5, 0.4 + (dataRange * 1.1))) // 0.3-1.5x (constrained)
  const bodyRatio = 0.7 + (avgValue * 0.3) // 0.7-1.0 (keeps body shape)
  const earSize = 0.5 + (dataRange * 0.5) // 0.5-1.0x (constrained)
