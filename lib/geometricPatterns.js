...
export function dataToPatternParams(fractalParams) {
  ...
  return {
    ...
    stripeWave: (columnAverages[3] || 0.5) * 0.6, // Wave effect strength
    
    // Checkerboard
    checkBlockSize: Math.floor(3 + (columnAverages[0] || 0.5) * 7),
    checkContrast: 0.8 + (columnAverages[1] || 0.5) * 0.4,
    checkDepth: 0.2 + (columnAverages[2] || 0.5) * 0.4, // 3D depth effect
