...
// Map data to pattern parameters using INDIVIDUAL VALUES and FINGERPRINT
export function dataToPatternParams(fractalParams) {
  if (!fractalParams) return {}
  
  const { columnAverages, variability, dataFingerprint, rowCount } = fractalParams
  
  // Use fingerprint to ensure different data = different patterns
  const fingerprintFactor = dataFingerprint ? (dataFingerprint % 1000) / 1000 : 0.5
  const rowCountFactor = rowCount ? Math.min(rowCount / 20, 1) : 0.5
  
  // DRAMATIC ranges with fingerprint influence for maximum visual differences
  return {
    // Gradient - Use fingerprint + individual values for DRAMATIC differences
    gradientAngle: Math.floor(((columnAverages[0] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 360), // 0° to 360° (continuous)
    gradientIntensity: 0.3 + ((columnAverages[1] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 1.7, // 0.3 to 2.0
    gradientOffset: ((columnAverages[2] || 0.5) * 0.7 + fingerprintFactor * 0.3 - 0.5) * 0.6, // -0.3 to 0.3
    
    // Random Grid - Use fingerprint as seed for UNIQUE patterns
    randomSeed: dataFingerprint || Math.floor((columnAverages[0] || 0.5) * 100000), // Use fingerprint directly
    randomDensity: 0.05 + ((variability || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.95, // 0.05 to 1.0
    
    // Spiral - DRAMATIC changes with fingerprint
    spiralFrequency: 0.5 + ((columnAverages[0] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 15, // 0.5 to 16
    spiralAmplitude: 0.2 + ((columnAverages[1] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 1.8, // 0.2 to 2.0
    spiralRotation: ((columnAverages[2] || 0.5) * 0.7 + fingerprintFactor * 0.3) * Math.PI * 6, // 0 to 6π
    
    // Stripes - DRAMATIC differences
    stripeWidth: Math.floor(1 + ((columnAverages[0] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 18), // 1 to 19
    stripeAngle: ((columnAverages[1] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 360, // 0 to 360°
    stripeContrast: 0.4 + ((columnAverages[2] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.8, // 0.4 to 1.2
    stripeWave: ((columnAverages[3] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 1.8, // 0 to 1.8
    
    // Checkerboard - DRAMATIC variation
    checkBlockSize: Math.floor(1 + ((columnAverages[0] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 18), // 1 to 19
    checkContrast: 0.5 + ((columnAverages[1] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.9, // 0.5 to 1.4
    checkDepth: 0.05 + ((columnAverages[2] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.95, // 0.05 to 1.0
    
    // Noise - DRAMATIC changes with fingerprint as seed
    noiseScale: 0.01 + ((columnAverages[0] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.4, // 0.01 to 0.41
    noiseOctaves: Math.floor(1 + ((columnAverages[1] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 8), // 1 to 9
    noisePersistence: 0.1 + ((columnAverages[2] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.9, // 0.1 to 1.0
    noiseSeed: dataFingerprint || Math.floor((columnAverages[3] || 0.5) * 100000), // Use fingerprint
    
    // Circular - DRAMATIC variation
    circularFrequency: Math.PI * 0.3 + ((columnAverages[0] || 0.5) * 0.7 + fingerprintFactor * 0.3) * Math.PI * 7, // 0.3π to 7.3π
    circularCenterX: 0.1 + ((columnAverages[1] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.8, // 0.1 to 0.9
    circularCenterY: 0.1 + ((columnAverages[2] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.8, // 0.1 to 0.9
    
    // Zigzag - DRAMATIC waves
    zigzagXFreq: 0.05 + ((columnAverages[0] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 1.5, // 0.05 to 1.55
    zigzagYFreq: 0.03 + ((columnAverages[1] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.8, // 0.03 to 0.83
    zigzagAmplitude: 0.3 + ((columnAverages[2] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 1.5, // 0.3 to 1.8
    
    // Diamond - DRAMATIC variation
    diamondScale: 0.3 + ((columnAverages[0] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 2.7, // 0.3 to 3.0
    diamondCenterX: 0.1 + ((columnAverages[1] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.8, // 0.1 to 0.9
    diamondCenterY: 0.1 + ((columnAverages[2] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.8 // 0.1 to 0.9
  }
}
