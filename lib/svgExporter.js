...
export function exportAnimal3DPatternSVG(animalPattern, animalModel) {
  ...
  // Page 1: 3D Model and pattern pieces
  let svg = `<svg width="${svgWidth}" height="${svgHeight}" xmlns="http://www.w3.org/2000/svg">`
  svg += `<defs>`
  ...
  // Variables for 3D model positioning
  let modelX = 50
  let modelY = 80
  let modelScale = 1.5
  
  // 3D Model Visualization
  if (animalModel && animalModel.parts) {
    svg += `<text x="20" y="60" font-family="Arial" font-size="14" font-weight="bold">3D Model Preview:</text>`
    
    // Draw 3D model
    animalModel.parts.forEach((part, index) => {
      if (part.type === 'ellipse') {
        svg += `<ellipse cx="${modelX + part.cx * modelScale}" cy="${modelY + part.cy * modelScale}" `
        svg += `rx="${part.rx * modelScale}" ry="${part.ry * modelScale}" `
        svg += `fill="${part.fill}" stroke="${part.stroke}" stroke-width="${part.strokeWidth}" `
        if (part.opacity !== undefined) svg += `opacity="${part.opacity}" `
        if (part.transform) svg += `transform="${part.transform}" `
        svg += `/>`
      } else if (part.type === 'circle') {
        svg += `<circle cx="${modelX + part.cx * modelScale}" cy="${modelY + part.cy * modelScale}" `
        svg += `r="${part.r * modelScale}" `
        svg += `fill="${part.fill}" `
        if (part.stroke) svg += `stroke="${part.stroke}" stroke-width="${part.strokeWidth || 0}" `
        svg += `/>`
      } else if (part.type === 'path') {
        // Adjust path coordinates
        const adjustedPath = part.d.replace(/([ML])\s+([-\d.]+)\s+([-\d.]+)/g, (match, cmd, x, y) => {
          return `${cmd} ${modelX + parseFloat(x) * modelScale} ${modelY + parseFloat(y) * modelScale}`
        })
        svg += `<path d="${adjustedPath}" `
        if (part.fill !== 'none') svg += `fill="${part.fill}" `
        svg += `stroke="${part.stroke}" stroke-width="${part.strokeWidth}" `
        if (part.opacity !== undefined) svg += `opacity="${part.opacity}" `
        if (part.strokeLinecap) svg += `stroke-linecap="${part.strokeLinecap}" `
        svg += `/>`
      }
    })
    
    svg += `<text x="20" y="${modelY + animalModel.height * modelScale + 30}" font-family="Arial" font-size="12" font-weight="bold">Pattern Pieces:</text>`
  } else {
    // No 3D model, show measurements at top
    svg += `<text x="20" y="60" font-family="Arial" font-size="12" font-weight="bold">Measurements:</text>`
    let yPos = 80
    ...
    svg += `<text x="20" y="${yPos + 10}" font-family="Arial" font-size="12" font-weight="bold">Pattern Pieces:</text>`
  }
  
  // Draw pattern pieces
  let currentY = animalModel && animalModel.parts ? modelY + animalModel.height * modelScale + 50 : 180
  let currentX = 50
