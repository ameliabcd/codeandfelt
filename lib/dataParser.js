...
export function dataToFractalParams(parsedData) {
  ...
  const params = {
    ...
    // Use fourth column for iterations (100 to 200 for more detail)
    maxIterations: columnAverages[3] ? Math.floor(100 + columnAverages[3] * 100) : 150,
...
export function generateDataColors(parsedData, numColors = 6) {
  ...
  // Generate colors with guaranteed contrast and vibrancy
  const colors = []
  
  // Use data to seed color generation, but ensure distinct colors
  const baseHue = (columnAverages[0] || 0.5) * 360
  const hueSpread = 70 + variability * 120 // More spread for varied data
  ...
  for (let i = 0; i < numColors; i++) {
    ...
    const saturationBase = i % 2 === 0 ? 80 : 95
    const saturation = saturationBase + (columnAverages[(i + 1) % columnAverages.length] || 0.5) * 5
    
    // Brighter, more vibrant lightness
    const lightnessBase = i % 2 === 0 ? 50 : 60
    const lightness = lightnessBase + (columnAverages[(i + 2) % columnAverages.length] || 0.5) * 10
