...
export function dataToAnimalParams(data, fractalParams) {
  if (!fractalParams) {
    ...
    // Return fixed default parameters (same as normal case)
    return {
      baseSize: 15,
      headRatio: 0.5,
      limbLength: 1.0,
  ...
  // FIXED sizes - don't change based on data, keep original animal shapes
  const baseSize = 15 // Fixed size (15cm)
  
  // Fixed proportions for each animal type - keep original shapes
  const headRatio = 0.5 // Fixed head ratio
  const limbLength = 1.0 // Fixed limb length (1x body)
  const roundness = 1.0 // Fixed roundness
  const bodyRatio = 0.9 // Fixed body ratio
  const earSize = 0.9 // Fixed ear size (0.9x head)
  const tailLength = 0.8 // Fixed tail length (0.8x body)
