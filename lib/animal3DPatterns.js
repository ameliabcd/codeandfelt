...
export function dataToAnimalParams(data, fractalParams) {
  if (!fractalParams) {
    console.warn('dataToAnimalParams: fractalParams is null or undefined')
  ...
  // Base size from data (10cm to 30cm) - LARGER animals
  const baseSize = 10 + ((columnAverages?.[0] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 20
  
  // Head to body ratio (0.45 to 0.55) - SMALLER variation, keep proportions
  const headRatio = 0.45 + ((columnAverages?.[1] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.1
  
  // Limb length (0.7x to 1.3x body) - SMALLER variation, keep proportions
  const limbLength = 0.7 + ((columnAverages?.[2] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.6
  
  // Roundness/curvature (0.8 to 1.2) - SMALLER variation
  const roundness = 0.8 + ((columnAverages?.[3] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.4
  
  // Body width to height ratio (0.8 to 1.0) - SMALLER variation, keep proportions
  const bodyRatio = 0.8 + ((columnAverages?.[4] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.2
  
  // Ear size (0.7x to 1.1x head) - SMALLER variation
  const earSize = 0.7 + ((columnAverages?.[5] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.4
  
  // Tail length (0.6x to 1.2x body) - SMALLER variation
  const tailLength = 0.6 + ((columnAverages?.[6] || 0.5) * 0.7 + fingerprintFactor * 0.3) * 0.6
