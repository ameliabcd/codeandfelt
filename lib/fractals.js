...
// Convert fractal pattern to color grid with enhanced vibrancy
export function fractalToColors(pattern, colors, invert = false) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      // Apply contrast enhancement for more vibrant fractals
      value = Math.pow(value, 0.8); // Enhance mid-tones
      
      // Map value to color index with better distribution
      // Use more of the color palette by scaling
      const scaledValue = value * (colors.length - 0.01);
      const colorIndex = Math.floor(scaledValue);
      const nextColorIndex = Math.min(colorIndex + 1, colors.length - 1);
      
      // Interpolate between colors for smoother gradients and more color usage
      const fraction = scaledValue - colorIndex;
      
      if (fraction > 0.5) {
        // Use more distinct color boundaries for vibrant look
        row.push(colors[nextColorIndex]);
      } else {
