...
// Koch Snowflake Generator (Optimized)
export function generateKochSnowflake(width, height, iterations = 3) {
  const pattern = Array(height).fill().map(() => Array(width).fill(0));
  
  // Limit iterations to prevent exponential slowdown
  const maxIterations = Math.min(iterations, 4);
  ...
  // Collect all line segments first (non-recursive approach for speed)
  const segments = [];
  ...
  segments.forEach(seg => {
    drawLine(pattern, seg.x1, seg.y1, seg.x2, seg.y2);
  });
